...
const Seller_newstoreRegistration = () => {
  ...
  const fetchSellerInfo = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        console.log("🔍 notificationPhotos 필드 존재:", 'notificationPhotos' in sellerInfo);
        ...
        console.log("🔍 sellerInfo.notificationPhotos 값:", sellerInfo.notificationPhotos);
        ...
        // 🔥 사진 데이터 확인 및 처리
        const existingStorePhotos = sellerInfo.sellerPhotos || sellerInfo.sellerPhotoUrls || [];
        ...
        // 나머지 처리 로직...
        const processedStoreImages = existingStorePhotos.map((photo, index) => {
          if (typeof photo === "string") {
            ...
            console.log(`✅ 매장 사진 ${index} - ID: ${photoId} (타입: ${typeof photoId}), URL: ${photoUrl}`);
        ...
        const processedNoticeImages = existingNoticePhotos.map((photo, index) => {
          if (typeof photo === "string") {
            ...
            console.log(`✅ 공지 사진 ${index} - ID: ${photoId} (타입: ${typeof photoId}), URL: ${photoUrl}`);
    ...
    if (index < originalStoreImages.length) {
      ...
      if (photoToDelete && photoToDelete.id !== null && photoToDelete.id !== undefined) {
        setDeletedStoreImageIds(prev => {
          ...
          console.log(`✅ 매장 사진 삭제 ID 추가: ${photoToDelete.id}, 새 목록:`, newList);
          return newList;
        });
      } else {
    ...
    if (index < originalNoticeImages.length) {
      ...
      if (photoToDelete && photoToDelete.id !== null && photoToDelete.id !== undefined) {
        setDeletedNoticeImageIds(prev => {
          const newList = [...prev, photoToDelete.id];
          console.log(`✅ 공지 사진 삭제 ID 추가: ${photoToDelete.id}, 새 목록:`, newList);
          return newList;
        });
      } else {
  ...
  const handleStoreImageChange = (index, e) => {
    ...
    if (file) {
      ...
      // 현재 슬롯이 새 파일인지 확인
      const isNewSlot = index >= originalStoreImages.length || storeImageFiles[index] instanceof File;
  ...
  const handleNoticeImageChange = (index, e) => {
    ...
    if (file) {
      ...
      const remainingExisting = originalNoticeImages.length - deletedNoticeImageIds.length;

      // 현재 슬롯이 새 파일인지 확인
      const isNewSlot = index >= originalNoticeImages.length || noticeImageFiles[index] instanceof File;
      const totalAfterAdd = remainingExisting + currentNewFiles + (isNewSlot ? 1 : 0);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const sellerInfo = {
        ...
        // 🔥 삭제할 사진 ID 목록 전송
        sellerPhotoIds: deletedStoreImageIds.map(id => {
          const numericId = parseInt(id);
          console.log(`🔥 매장 사진 ID 변환: ${id} -> ${numericId}`);
          return numericId;
        }).filter(id => !isNaN(id)),

        notificationPhotoIds: deletedNoticeImageIds.map(id => {
          const numericId = parseInt(id);
      ...
      // 새로 업로드된 파일들만 전송
      const newStoreFiles = storeImageFiles.filter((file) => file instanceof File && file.size > 0);
      const newNoticeFiles = noticeImageFiles.filter((file) => file instanceof File && file.size > 0);
...
일요일 09:00 ~ 18:00"
              ...
              <section className="seller-info-card">
                <div className="seller-card-header">
                  ...
                  <p className="seller-card-subtitle">
                    매장의 모습을 보여주는 사진을 업로드하세요 (최대 5장) - 현재: {getTotalStoreImageCount()}/5
                  </p>
                </div>
              ...
              <section className="seller-info-card">
                <div className="seller-card-header">
                  ...
                  <p className="seller-card-subtitle">
                    고객에게 전달하고 싶은 공지사항을 작성하세요 (최대 5장) - 현재: {getTotalNoticeImageCount()}/5
                  </p>
                </div>
