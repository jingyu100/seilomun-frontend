...
const Seller_RefundDetail = () => {
    ...
    const fetchRefundDetail = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`http://3.39.239.179/api/sellers/refunds/${refundId}`, {
    ...
    const handleAcceptRefund = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                `http://3.39.239.179/api/orders/refund/acceptance/${refundId}`,
                {},
                { withCredentials: true }
            );
    ...
    const handleDeclineRefund = async () => {
        ...
        try {
            setActionLoading(true);
            const response = await axios.post(
                `http://3.39.239.179/api/orders/refund/decline/${refundId}`,
                {},
                { withCredentials: true }
            );
    ...
    const getRefundStatusClass = (status) => {
        switch (status) {
            case 'N': return 'refund-status-waiting';
            case 'A': return 'refund-status-approved';
            case 'R': return 'refund-status-rejected';
            default: return 'refund-status-unknown';
        }
    };
                ...
                <div className="refund-detail-container">
                    ...
                    <div className="refund-detail-header">
                        ...
                        <div className="refund-status">
                            <span className={`status-badge ${getRefundStatusClass(refundDetail.refundStatus)}`}>
                    ...
                    <div className="refund-info-card">
                        ...
                        <div className="refund-info-grid">
                            ...
                            <div className="info-item">
                                ...
                                <span className="value">{formatDate(refundDetail.refundRequestDate)}</span>
                            </div>
                            <div className="info-item">
                                    ...
                                    {refundDetail.refundPhotos.map((photo, index) => (
                                        <div key={index} className="photo-item">
                                            <img
                                                ...
                                                alt={`환불 첨부 ${index + 1}`}
                                                onClick={() => window.open(`https://seilomun-bucket.s3.ap-northeast-2.amazonaws.com/${photo}`, '_blank')}
                                            />
                                        </div>
                                    ))}
                                </div>
                    ...
                    <div className="order-info-card">
                        ...
                        <div className="order-info-grid">
                            ...
                            <div className="info-item">
                                ...
                                <span className="value">{getOrderStatusText(refundDetail.orderStatus)}</span>
                            </div>
                            <div className="info-item">
                    ...
                    <div className="payment-info-card">
                        ...
                        <div className="payment-breakdown">
                            <div className="payment-row">
                                <span>상품 금액:</span>
                                <span>{(refundDetail.totalAmount - (refundDetail.deliveryFee || 0) + refundDetail.usedPoints)?.toLocaleString()}원</span>
                            </div>
