...
const Seller_ProductManagement = () => {
    ...
    const fetchProducts = async () => {
        try {
            setLoading(true);
            // 판매자 정보 먼저 조회
            const sellerResponse = await axios.get("http://localhost/api/sellers", {
            ...
            // 해당 판매자의 상품 목록 조회
            const productsResponse = await axios.get(`http://localhost/api/products/seller/${sellerId}`, {
                withCredentials: true,
            });
    ...
    const handleStatusChange = async (productId, newStatus) => {
        try {
            await axios.put(`http://localhost/api/products/${productId}/status`,
                { status: newStatus },
                { withCredentials: true }
            );
